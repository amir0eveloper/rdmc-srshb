...
export default async function SearchPage({
  ...
  let items = [];
  let totalItems = 0;
  const where: Prisma.ItemWhereInput = { status: "PUBLISHED" };
  let description = "Browsing all published items.";
  let searchUrlParams = "";

  if (advancedQuery) {
    try {
      const queryElements: QueryElement[] = JSON.parse(
        decodeURIComponent(advancedQuery)
      );
      const advancedWhere = buildAdvancedWhereClause(queryElements);
      where.AND = [
        ...((where.AND as Prisma.ItemWhereInput[]) || []),
        ...((advancedWhere.AND as Prisma.ItemWhereInput[]) || []),
      ];
    ...
    searchUrlParams = `query=${encodeURIComponent(
      query
                ...
                return (
                  <div key={item.id} className="py-6">
                    <Link href={`/items/${item.id}`}>
                      <h3 className="text-xl text-blue-600 hover:underline cursor-pointer font-semibold">
                        {item.title}
                      </h3>
                    </Link>
